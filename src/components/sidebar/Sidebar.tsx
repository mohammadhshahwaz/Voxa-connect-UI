  ...
  return (
    <aside className="w-64 flex-shrink-0 bg-[#102d0b] text-[#d3ffcd] flex flex-col justify-between">
      ...
      <nav className="flex-1 flex flex-col justify-center px-4 space-y-2">
        ...
        {/* <NavLink to="/calldemo" className={linkClasses}>
          📞 Demo Call
        </NavLink> */}
      </nav>

      {/* User Profile */}
      <div ref={dropdownRef} className="p-4 border-t  border-green-800 relative">
        {/* Button */}
        <button
          onClick={() => setOpen(!open)}
          className="flex items-center justify-between w-full px-3 py-2 rounded-lg bg-[#d3ffcd] text-[#102d0b] font-medium"
        >
        ...
        {/* Dropdown Menu */}
        {open && (
          <div className="absolute top-[-50px] border-2 rounded-lg  mt-2 w-56 bg-[#102d0b] z-50">
            <button className="block w-full text-left px-4 py-2 text-md text-[#d4ffcd] hover:text-[#102d0b] hover:bg-[#d4ffcd]">
              Logout
            </button>
          </div>
        )}
