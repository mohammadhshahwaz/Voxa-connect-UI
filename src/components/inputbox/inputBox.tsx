...
const InputBox: React.FC<InputBoxProps> = ({ onMessageSent }) => {
  ...
  useEffect(() => {
    ...
    const auth = getPersistedAuth<{ user_id?: string; username?: string }>();
    if (auth?.user_id) {
      userIdRef.current = auth.user_id;
      if (auth.username) usernameRef.current = auth.username;
      return;
    }
  ...
  useEffect(() => {
    ...
    return (
        <div>
            {selectedFiles.length > 0 && (
                <div className="mx-3 mb-2 flex flex-wrap gap-2">
                    {selectedFiles.map((file, index) => (
                        <div
                            key={index}
                            className="flex items-center gap-2 bg-[var(--bg-white)] px-3 py-1 rounded-md border border-gray-300"
                        >
            ...
            <div className="m-3 flex sticky bottom-0 items-center gap-3 border-2 bg-[var(--color-primary)] text-[var(--color-primary-dark)] font-semibold rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-gray-400">
                <input
                    type="text"
                ...
                >
                    {isLoading ? (
                        <div className="w-10 h-10 flex items-center justify-center">
                            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[-var(--color-primary-dark)]"></div>
                        </div>
                    ) : (
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
