...
import MarkdownRenderer from "../../hooks/formatResponse";
...
const Conversation: React.FC = () => {
  ...
  // 🔥 Buffer to collect full AI message during streaming
  let finalBuffer = "";
  ...
  const handleMessageSent = ({
    ...
    // 📌 STREAMING CHUNKS
    if (response) {
      const incomingText = response.content || "";
      finalBuffer += incomingText; // append chunk to buffer
      ...
      setMessages((prev) => {
        ...
        if (lastAi && lastAi.message_id === response.message_id) {
          ...
          return prev.map((m) => {
            ...
            const nextText = existing + incomingText; // preserve exact markdown
    ...
    // 📌 NORMAL (NON-STREAMING) AI RESPONSE
    if (aiResponse) {
      newMessages.push({
        sender: "ai",
        text: aiResponse,
